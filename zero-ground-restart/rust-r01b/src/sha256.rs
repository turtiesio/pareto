//! Small, dependency-free SHA-256 implementation for frozen hashes.
...
impl Sha256 {
    ...
    fn compress(&mut self, block: &[u8; 64]) {
        let mut w = [0_u32; 64];
        for (word, chunk) in w[..16].iter_mut().zip(block.as_chunks::<4>().0.iter()) {
...
mod tests {
    ...
    #[test]
    fn standard_vectors_and_incremental_updates() {
        assert_eq!(
            hex::encode(&digest(b"")),
        ...
        assert_eq!(hash.finalize(), digest(b"abc"));
    }
}

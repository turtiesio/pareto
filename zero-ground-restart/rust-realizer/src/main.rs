...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match (command, arguments.as_slice()) {
        ...
        ("consume", [_, candidate, path]) => {
            ...
            println!(
                "candidate={} records={} bytes={} state_stream_sha256={} unique_recovered_classes={}",
                summary.candidate.name(),
                summary.records,
                summary.bytes,
                summary.sha256,
...
fn usage<T>() -> Result<T, Box<dyn std::error::Error>> {
    Err("usage: b1-realizer inspect | produce <ordinal|representative> <path|-> | consume <ordinal|representative> <path|->".into())
}
